...
(function (global) {
  ...
  // Format a parsed UTC instant as YYYYMMDDTHHMMSSZ.
  ...
  function buildEventLines(assignment) {
    const due = parseCourseDateTime(assignment.dueISO);
    if (!due) return null;

    const courseName = (assignment.courseName || "").trim() || "Course";
    const tzMode = assignment.tzMode === "utc" ? "utc" : "preserve";
    ...
    const startLocal = addMinutes(due, -120).local;
    const endLocal = due.local;
    const uid =
      djb2(courseName + "|" + assignment.name + "|" + assignment.dueISO) +
      "-ics@local";

    const descParts = ["Course: " + courseName];
    if (assignment.week) descParts.push("Week: " + assignment.week);
    if (assignment.type) descParts.push("Type: " + assignment.type);
    if (assignment.status) descParts.push("Status: " + assignment.status);
    if (assignment.meta) descParts.push("Meta: " + assignment.meta);
    if (assignment.releasedText) descParts.push("Released: " + assignment.releasedText);
    if (assignment.dueLocalLabel) {
      descParts.push("Original deadline: " + assignment.dueLocalLabel);
    }
    ...
    if (tzMode === "utc") {
      // due is a local wall-clock time. Convert to UTC using its offset.
      const dueUTC = parsedToUTC(due);
      const startUTC = new Date(dueUTC.getTime() - 120 * 60 * 1000);
      lines.push("DTSTART:" + formatUTCCompact(startUTC));
      lines.push("DTEND:" + formatUTCCompact(dueUTC));
    } else {
      lines.push("DTSTART;TZID=" + TZID + ":" + startLocal);
      lines.push("DTEND;TZID=" + TZID + ":" + endLocal);
    }

    const summary = assignment.week
      ? courseName + " — " + assignment.week + " — " + assignment.name
    ...
    lines.push("CATEGORIES:" + escapeText(assignment.source || "Course") + "," + escapeText(courseName));
  ...
  // Accepts either:
  //   - a flat array of assignments: buildICS([{ courseName, name, dueISO, tzMode, ... }, ...])
  //   - the legacy object form: buildICS({ courseName, courseId, assignments })
  function buildICS(input) {
    ...
    if (Array.isArray(input)) {
      ...
      calNameHint = input.courseName;
    } else {
      throw new Error("buildICS: expected an array of assignments.");
    }
    ...
    // Detect sources and tz modes present in this batch.
    const sources = [
      ...new Set(assignments.map((a) => a.source || "Course").filter(Boolean)),
    ];
    const needsVtimezone = assignments.some((a) => a.tzMode !== "utc");

    // Calendar display name: single course → that course; multi → summary.
    const uniqueCourses = [
      ...new Set(assignments.map((a) => (a.courseName || "").trim()).filter(Boolean)),
    ];
    let calName;
    if (calNameHint) {
      calName = calNameHint.trim();
    } else if (uniqueCourses.length === 1) {
      calName = uniqueCourses[0];
    } else if (uniqueCourses.length > 1) {
      const tag = sources.length === 1 ? sources[0] : "Courses";
      calName = tag + " (" + uniqueCourses.length + ")";
    } else {
      calName = sources[0] || "Calendar";
    }
